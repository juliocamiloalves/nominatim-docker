...
if [[ "$PBF_URL" = "" && "$PBF_PATH" = "" ]]  ||  [[ "$PBF_URL" != "" && "$PBF_PATH" != "" ]]; then
    ...
    echo "docker run -e PBF_URL=https://download.geofabrik.de/south-america/brazil-latest.osm.pbf ..."
    echo "docker run -e PBF_PATH=/nominatim/data/brazil-latest.osm.pbf ..."
    exit 1
fi
